...
OrthographicCamera::OrthographicCamera(float left, float right, float bottom, float top)
  ...
  projection_matrix_ = mEn::Mat4(1);  // TODO mEn::ortho(left, right, bottom, top, -1.0F, 1.0F);
...
bool OrthographicCamera::on_window_resize(WindowResizeEvent& event) {
  ...
  left_              = -aspect;
  right_             = aspect;
  projection_matrix_ = mEn::Mat4(1);  // TODO mEn::ortho(left_, right_, bottom_, top_, -1.0F, 1.0F);
...
PerspectiveCamera::PerspectiveCamera(float fov, float aspect, float zNear, float zFar)
  ...
  projection_matrix_ = mEn::Mat4(1);  // TODO mEn::perspective(fov, aspect, zNear, zFar);
  dispatcher_.subscribe<kEn::WindowResizeEvent>(KEN_EVENT_SUBSCRIBER(on_window_resize));
}
...
bool PerspectiveCamera::on_window_resize(WindowResizeEvent& event) {
  ...
  projection_matrix_ = mEn::Mat4(1);  // TODO mEn::perspective(fov_, aspect_, zNear_, zFar_);
