...
namespace kEn {
game_object::game_object(glm::vec3 pos, glm::quat rot, glm::vec3 scale) : transform_(pos, rot, scale) {}
...
void game_object::render_all(shader& shader) const {
  ...
  for (const auto child : children_) child.get().render_all(shader);
}
...
void game_object::update_all(double delta) {
  ...
  for (const auto child : children_) child.get().update_all(delta);
}
