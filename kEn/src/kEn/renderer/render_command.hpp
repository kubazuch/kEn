...
class render_command {
 public:
  ...
  static void draw_indexed(const vertex_array& vertex_array, uint32_t index_count = 0) {
    renderer_api_->draw_indexed(vertex_array, index_count);
  }
  static void draw_patches(const vertex_array& vertex_array, uint32_t vertex_count = 0) {
