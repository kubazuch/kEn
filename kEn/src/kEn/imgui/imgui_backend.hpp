...
/**
 * Abstract interface for the Dear ImGui renderer backend.
 *
 ...
 * renderer init path (e.g. InitForOpenGL vs InitForVulkan).
 *
...
class ImguiBackend {
 public:
  ...
  /** Initializes the platform and renderer ImGui backends. */
  virtual void init(GLFWwindow* window) = 0;
  /** Shuts down the platform and renderer ImGui backends. */
  virtual void shutdown() = 0;
  /** Calls the backend New Frame functions before ImGui::NewFrame(). */
  virtual void new_frame() = 0;
  /** Submits accumulated draw data and handles multi-viewport rendering. */
