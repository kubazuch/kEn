...
transform::~transform() {
  ...
  if (parent_.has_value()) unset_parent();
}
...
glm::mat4& transform::local_to_world_matrix() const {
  ...
  if (parent_) model_mat_ = parent_.value().get().local_to_world_matrix() * model_mat_;
...
void transform::model_matrix_updated() {
  ...
  if (parent_) local_mat_ = parent_.value().get().world_to_local_matrix() * local_mat_;
