...
namespace kEn
{
	...
	public:
		...
		glm::vec3 pos() const { return parent_.has_value() ? glm::vec3(parent_.value().get().local_to_world_matrix() * glm::vec4(pos_, 1.f)) : pos_; }
		void set_local_pos(const glm::vec3& pos) { pos_ = pos; set_dirty(); }
		...
		void set_local_rot(const glm::quat& rot) { rot_ = rot; set_dirty(); }

		const glm::vec3& local_scale() const { return scale_; }
		glm::vec3& local_scale() { return scale_; }
		void set_local_scale(const glm::vec3& scale) { scale_ = scale; set_dirty(); }
		void set_local_scale(float scale) { scale_ = glm::vec3(scale); set_dirty(); }
