...
void attach_color_texture(uint32_t id, int samples, TextureFormat fmt, uint32_t width, uint32_t height, size_t index) {
  const bool multisampled = samples > 1;
  const auto gl           = texture_format::opengl_describe(fmt);
  if (multisampled) {
    glTexImage2DMultisample(GL_TEXTURE_2D_MULTISAMPLE, samples, static_cast<GLenum>(gl.internal_format),
                            static_cast<GLsizei>(width), static_cast<GLsizei>(height), GL_FALSE);
  } else {
    glTexImage2D(GL_TEXTURE_2D, 0, gl.internal_format, static_cast<GLsizei>(width), static_cast<GLsizei>(height), 0,
                 gl.pixel_format, gl.pixel_type, nullptr);
...
void attach_depth_texture(uint32_t id, int samples, TextureFormat fmt, uint32_t width, uint32_t height) {
  const bool multisampled = samples > 1;
  const auto gl           = texture_format::opengl_describe(fmt);
  if (multisampled) {
    glTexImage2DMultisample(GL_TEXTURE_2D_MULTISAMPLE, samples, static_cast<GLenum>(gl.internal_format),
    ...
    glTexStorage2D(GL_TEXTURE_2D, 1, static_cast<GLenum>(gl.internal_format), static_cast<GLsizei>(width),
...
void OpenglFramebuffer::invalidate() {
  ...
  if (!color_attachment_specs_.empty()) {
    ...
    for (size_t i = 0; i < color_attachments_.size(); i++) {
      ...
      attach_color_texture(color_attachments_[i], 1, color_attachment_specs_[i].texture_format, spec_.width,
                           spec_.height, i);
    }
  }
  ...
  if (depth_attachment_spec_.has_value()) {
    ...
    attach_depth_texture(depth_attachment_, 1, depth_attachment_spec_->texture_format, spec_.width, spec_.height);
  }
...
void OpenglFramebuffer::unbind() {
  ...
  const mEn::Vec2 window_size = kEn::Application::instance().main_window().size();
...
void OpenglFramebuffer::clear_attachment(uint32_t attachment_id, int value) {
  ...
  const auto fmt = color_attachment_specs_[attachment_id].texture_format;
  glClearTexImage(color_attachments_[attachment_id], 0, texture_format::pixel_format(fmt), GL_INT, &value);
}
...
void OpenglFramebuffer::clear_attachment(uint32_t attachment_id, float value) {
  ...
  const auto fmt = color_attachment_specs_[attachment_id].texture_format;
  glClearTexImage(color_attachments_[attachment_id], 0, texture_format::pixel_format(fmt), GL_FLOAT, &value);
}
...
void OpenglFramebuffer::clear_attachment(uint32_t attachment_id, mEn::Vec4 value) {
  ...
  const auto fmt = color_attachment_specs_[attachment_id].texture_format;
  glClearTexImage(color_attachments_[attachment_id], 0, texture_format::pixel_format(fmt), GL_FLOAT,
                  mEn::value_ptr(value));
}
