...
opengl_vertex_array::opengl_vertex_array() { glGenVertexArrays(1, &renderer_id_); }

opengl_vertex_array::~opengl_vertex_array() { glDeleteVertexArrays(1, &renderer_id_); }

void opengl_vertex_array::bind() const { glBindVertexArray(renderer_id_); }

void opengl_vertex_array::unbind() const { glBindVertexArray(0); }
  ...
  for (const auto& element : layout) {
    switch (element.type) {
      ...
      case shader_data_types::float4: {
        ...
        glVertexAttribPointer(vertex_buffer_index_, shader_data_types::get_component_count(element.type),
                              get_opengl_type(element.type), element.normalized ? GL_TRUE : GL_FALSE, layout.stride(),
      ...
      case shader_data_types::bool_: {
        glEnableVertexAttribArray(vertex_buffer_index_);
        glVertexAttribIPointer(vertex_buffer_index_, shader_data_types::get_component_count(element.type),
                               get_opengl_type(element.type), layout.stride(), (const void*)element.offset);
        vertex_buffer_index_++;
        break;
      }
      ...
      case shader_data_types::mat4: {
        ...
        for (uint8_t i = 0; i < count; ++i) {
          glEnableVertexAttribArray(vertex_buffer_index_);
          glVertexAttribPointer(vertex_buffer_index_, count, get_opengl_type(element.type),
