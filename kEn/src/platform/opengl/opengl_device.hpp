...
class OpenglDevice final : public Device {
 public:
  ...
  RenderContext& context() override { return render_context_; }
  ...
  std::shared_ptr<Shader> create_compute_shader(const std::filesystem::path&) override;
 ...
 private:
  OpenglSwapChain swap_chain_;
  OpenglRenderContext render_context_;
