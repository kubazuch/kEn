...
// A linear solve accumulates round-off, so compare against the known solution with
// a precision-appropriate tolerance rather than the fixed-epsilon EXPECT_VEC*_EQ.
...
// Build an invertible NxN matrix (column-major glm args), matching the ones used
...
// A matrix whose first diagonal entry is zero forces a row swap during pivoting.
template <mEn::length_t N, typename T>
...
// A singular matrix (one column is a multiple of another) must report failure.
template <mEn::length_t N, typename T>
